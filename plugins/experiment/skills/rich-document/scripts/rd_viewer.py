...
from rd_core import Failure, check_id


def runtime_root() -> Path:
    identity = str(os.getuid()) if hasattr(os, "getuid") else os.environ.get("USERNAME", "user")
    path = Path(tempfile.gettempdir()) / ("rich-document-runtime-" + hashlib.sha256(identity.encode()).hexdigest()[:12])
...
def runtime_file(artifact_id: str) -> Path:
    check_id(artifact_id)
    return runtime_root() / (artifact_id + ".json")


def load_runtime(artifact_id: str) -> dict | None:
    path = runtime_file(artifact_id)
    try:
        ...
        if url.scheme != "http" or url.hostname != "127.0.0.1" or not url.port or url.path or url.query or url.username:
            return None
        if record.get("id") != artifact_id or not isinstance(record.get("token"), str):
            return None
        return record
...
def view_result(record: dict) -> dict:
    ...
    return {"status": "ready", "artifact_id": record["id"], "saved": record["saved"],
...
def start(artifact: Path, entrypoint: Path) -> dict:
    ...
    with viewer_lock(artifact_id):
        existing = load_runtime(artifact_id)
        if healthy(existing) and Path(existing["artifact"]).resolve() == artifact.resolve():
            return view_result(existing)
        runtime_file(artifact_id).unlink(missing_ok=True)
        log = runtime_root() / (artifact_id + ".log")
        ...
        try:
            ...
            while time.monotonic() < deadline:
                record = load_runtime(artifact_id)
                if healthy(record):
            ...
            raise Failure(5, "viewer", "Viewer did not become ready. Use serve ID in the harness's background runner. " + detail,
                          artifact_id=artifact_id, html_path=str(artifact / "rendered/index.html"), saved=manifest["saved"])
        except OSError as exc:
            raise Failure(5, "viewer", f"Cannot start viewer: {exc}", artifact_id=artifact_id,
                          html_path=str(artifact / "rendered/index.html"), saved=manifest["saved"]) from exc


def stop(artifact_id: str) -> dict:
    with viewer_lock(artifact_id):
        record = load_runtime(artifact_id)
        if not healthy(record):
            runtime_file(artifact_id).unlink(missing_ok=True)
            return {"status": "stopped", "artifact_id": artifact_id, "already_stopped": True}
        request(record, "/stop", "POST")
        for _ in range(100):
            if not healthy(record):
                return {"status": "stopped", "artifact_id": artifact_id, "saved": record["saved"]}
            time.sleep(0.05)
...
def serve(artifact: Path, *, guard_startup: bool = False) -> None:
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path in {"/", "/index.html"}:
                ...
                self.reply(200, json.dumps({"id": artifact_id}).encode(), "application/json")
            else:
    ...
    with viewer_lock(artifact_id) if guard_startup else contextlib.nullcontext():
        existing = load_runtime(artifact_id)
        if healthy(existing):
        ...
        record = {"id": artifact_id, "pid": os.getpid(), "base": f"http://127.0.0.1:{server.server_port}",
    ...
    try:
        ...
        server.server_close()
        current = load_runtime(artifact_id)
