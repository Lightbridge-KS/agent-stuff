...
**Default: off.** Do this *only* when the user explicitly asks for it (e.g. "include the
tests", "pair the unit tests", "show each test variation live"). When off, ignore tests
entirely. When on, fold them in *in addition to* the normal mock define→run→show cells —
