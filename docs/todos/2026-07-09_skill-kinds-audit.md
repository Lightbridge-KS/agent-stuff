...
- **P3 dead-weight trims (biggest first):** ~~c4-architect~~ ✅ done 2026-07-09 (references tier deleted, context-map glosses deduped to the SKILL.md table, description → trigger phrase; 1211 → 487 lines); explain-family shared scaffolding (~60–80 lines × 4 doc-siblings — factor into one shared contract or trim per file; keep the filename/cross-link protocol coordinated); writing-great-skills SKILL↔GLOSSARY duplication (~40–50 lines); research `modules/general-web.md` + parts of `report-style.md`; ax-interface-analysis generic rubric; codebase-design testability snippets; explain-as-notebook test-pairing block.
